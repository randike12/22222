...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    welcome_message = """
❄️ *WELCOME TO @RAJPUTDDOS DDOS WORLD❄️

🔥 Yeh bot apko deta hai hacking ke maidan mein asli mazza! 🔥

✨ *Key Features:* ✨
🚀 𝘼𝙩𝙩𝙖𝙘𝙠 𝙠𝙖𝙧𝙤 𝙖𝙥𝙣𝙚 𝙤𝙥𝙥𝙤𝙣𝙚𝙣𝙩𝙨 𝙥𝙖𝙧 𝘽𝙜𝙢𝙞 𝙈𝙚 /rajput
...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in user_data:
        await update.message.reply_text(
            "💰 Bhai, tere paas toh coins nahi hai! Pehle owner ke paas ja aur coins le aa. 😂 OWNER :- @RAJPUTDDOS "
        )
        return
...
👉 `/rajput <ip> <port> <duration>`
📌 *Example:* `/rajput 192.168.1.1 26547 240`
"""
